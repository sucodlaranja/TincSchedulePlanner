...
export default function ShiftConfigForm() {
  ...
  function handleAdd() {
    ...
    setForm({
      ...
      startTime: "08:00",
      endTime: "16:00",
      color: SHIFT_COLORS[shifts.length + 1] ?? SHIFT_COLORS[0],
      activeDays: [],
    });
    setAdding(false);
  }
              ...
              {DAY_CHIPS.map(({ label, value }) => (
                <button
                  ...
                  onClick={() =>
                    setForm((f) => ({
                      ...f,
                      activeDays: toggleDay(f.activeDays, value),
                    }))
                  }
...
function ShiftRow({
  ...
  const [form, setForm] = useState({
    ...
    color: shift.color,
    activeDays: (shift.activeDays ?? []) as DayOfWeek[],
  });
      ...
      <div className="flex-1 min-w-0">
        ...
        {shift.activeDays && shift.activeDays.length > 0 && (
          <p className="text-xs text-indigo-500 mt-0.5">
            {DAY_CHIPS.filter(({ value }) => shift.activeDays.includes(value))
              .map(({ label }) => label)
              .join(", ")}
          </p>
        )}
