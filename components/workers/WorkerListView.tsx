...
export default function WorkerListView() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4">
              ...
              {workers.map((worker) => (
                <tr key={worker.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell text-gray-600">
                    {worker.hoursPerWeek}h
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell text-gray-500 text-xs">
                    {(worker.preferences.preferredDaysOff ?? []).length > 0
                      ? (worker.preferences.preferredDaysOff ?? [])
                          .map(
                            (d) =>
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${worker.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
      ...
      >
        {editWorker && (
          <WorkerForm worker={editWorker} onClose={() => setEditWorker(null)} />
        )}
      </Modal>
    </div>
  );
